...
class MappingWithKnownPoses(Node):
    def __init__(self, name):
        super().__init__(name)
        self.declare_parameter("width", 50.0) ##The width and height of our occupnacy grid 50m x 50m
        self.declare_parameter("height", 50.0)
        self.declare_parameter("resolution", 0.1) # The reoslution of 10cms
        ...
        self.map_.info.width = round(width / resolution) ##The width and height of our occupnacy grid is mentioned in no. of cells
        self.map_.info.height = round(height / resolution) ## as the OccupancyGrid class needs that info instead on the size in meters
        self.map_.info.origin.position.x = float(-round(width / 2.0)) # The origin can be the location of the cell in the center of the map
        self.map_.info.origin.position.y = float(-round(height / 2.0))
        self.map_.header.frame_id = "odom"
        self.map_.data = [-1] * (self.map_.info.width * self.map_.info.height) ##The total length of the vector of occupany grid. 
        ##Here -1 is saying that initially we assume the entore map is unknnow.
        self.probability_map_ = [prob2logodds(PRIOR_PROB)] * (self.map_.info.width * self.map_.info.height)

        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self) #Used for knowing the transnformation of the robot to know its current poistion.
    ...
    def scanCallback(self, scan: LaserScan):
        try:
            t = self.tf_buffer.lookup_transform(self.map_.header.frame_id, scan.header.frame_id, rclpy.time.Time()) ##This contains the main buffer transform data between the frames odom and scanner
